...
export class FuseDateRangeComponent implements ControlValueAccessor, OnDestroy {
  @Output() readonly rangeChanged: EventEmitter<{ start: string; end: string }> = new EventEmitter<{
    start: string;
    end: string;
  }>();
  ...
  dateFilter(): any {
    ...
    return (date: Moment): boolean =>
      !(this.setWhichDate === 'end' && date.isBefore(this._range.start, 'day'));
  }
